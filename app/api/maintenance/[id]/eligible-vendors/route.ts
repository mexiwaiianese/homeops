...
export async function GET(
  ...
  if (!supabase)
    return NextResponse.json({
      mode: "demo",
      vendors: demoVendors.map((v) => ({
        ...v,
        eligibility: explainVendorEligibility(v),
      })),
    });
  ...
  const vendors = await Promise.all((data ?? []).map(async(v) => {
    ...
    return {...v,eligibility:eligibilityError?explainVendorEligibility(v,{homeId:req.home_id,serviceCategoryId:req.service_category_id}):eligibility};
  }));
  return NextResponse.json({ mode: "live", vendors });
}
