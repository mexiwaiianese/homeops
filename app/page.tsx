  ...
  return (
    <main className="shell">
      ...
      <section className="content">
        ...
        {tab === "Today" && <Today maintenance={maintenance} onAdvance={nextStatus} collected={collected} homes={homes} tenants={tenants} />}
        ...
        {tab === "Maintenance" && <Maintenance rows={maintenance} homes={homes} onAdvance={nextStatus} />}
      </section>
...
function Today({ maintenance, onAdvance, collected, homes, tenants }: { maintenance: MaintenanceUI[]; onAdvance: (id: string) => void; collected: number; homes: HomeUI[]; tenants: TenantUI[] }) {
  return <>
    <div className="stats">
...
function Task({ item, homes, onAdvance }: { item: MaintenanceUI; homes: HomeUI[]; onAdvance: (id: string) => void }) { const h = homes.find((x) => x.id === item.homeId); return <div className="task"><div className={`severity ${item.priority.toLowerCase()}`} /><div className="taskMain"><div className="taskTitle"><strong>{item.title}</strong><span className={`tag ${item.priority.toLowerCase()}`}>{item.priority}</span></div><p>{h?.address ?? "Home"} • {item.tenant}</p><small>{item.note}</small></div><div className="taskAction"><span>{item.status}</span>{item.estimate > 0 && <strong>{money(item.estimate)}</strong>}<button onClick={() => onAdvance(item.id)} disabled={item.status === "Documented"}>{item.status === "Authorize" ? "Approve & continue" : item.status === "Documented" ? "Complete" : "Advance"}</button></div></div>; }
...
function Maintenance({ rows, homes, onAdvance }: { rows: MaintenanceUI[]; homes: HomeUI[]; onAdvance: (id: string) => void }) { const flow = ["Diagnose","Authorize","Dispatch","Scheduled","Repair","Invoice","Documented"]; return <><section className="panel"><div className="panelHead"><div><p className="eyebrow">FLAGSHIP WORKFLOW</p><h2>Maintenance command center</h2></div><span className="pill">Request → Documented</span></div><div className="flow">{flow.map((s,i)=><div key={s}><b>{i+1}</b><span>{s}</span></div>)}</div></section><section className="panel"><div className="taskList">{rows.map(r => <Task key={r.id} item={r} homes={homes} onAdvance={onAdvance} />)}</div></section></>; }
